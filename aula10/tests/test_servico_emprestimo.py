...
def test_registrar_notifica_usuario_apos_sucesso(servico, notificador_spy):
    ...
    # Assert — o Spy registrou exatamente um evento de empréstimo
    assert len(notificador_spy.eventos) == 1
    tipo, email, _data = notificador_spy.eventos[0]
    assert tipo == "emprestimo"
    assert email == "ana@ufra.edu.br"
...
def test_devolver_calcula_multa_correta_para_atraso(
    ...
    # Act
    servico.devolver(1)
    # Assert — a multa chega ao usuário via notificação de devolução
    devolucoes = [e for e in notificador_spy.eventos if e[0] == "devolucao"]
    assert devolucoes[-1][2] == multa_esperada
...
def test_devolver_falha_silenciosamente_para_emprestimo_inexistente(servico, notificador_spy):
    ...
    # Assert — nenhuma notificação de devolução foi emitida
    assert all(e[0] != "devolucao" for e in notificador_spy.eventos)

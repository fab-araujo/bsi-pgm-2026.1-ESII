...
**Strategy × herança da Aula 5.** Na Aula 5 o cálculo de multa foi para as
subclasses porque a única variação que existia era "por tipo de equipamento" —
e herança resolve bem uma dimensão única. A limitação aparece quando a política
de multa deixa de depender do tipo: uma multa progressiva ou por hora não cabe
na hierarquia sem virar subclasse nova para cada combinação tipo × política.
Movendo o algoritmo para `MultaStrategy`, a variação ganhou eixo próprio: o
`Notebook` usa `MultaPorDia(10.0)` hoje e poderia usar `MultaProgressiva`
amanhã, sem tocar em `Equipamento` — que virou Context e apenas delega. O OCP
da Aula 5 não foi desfeito; mudou de mecanismo: o sistema continua aberto a
extensão (política nova = classe nova, como a `MultaProgressiva` que testei
isolada), mas agora por composição, não por herança — exatamente o "prefira
composição a herança" de Valente (Cap. 6).

**Observer e o evento-dict.** Antes, o `ServicoEmprestimo` chamava
`notificador.notificar_*` diretamente: conhecia o destino (SRP ferido) e
precisava mudar a cada destino novo (OCP ferido). Como Subject, ele apenas
anuncia `self.notificar({...})`; o `NotificadorEmail` escuta hoje, um
`LogObserver` escutaria amanhã — e o serviço não muda. O DIP também melhora:
o serviço depende só da abstração `Observer`. O evento como `dict`, porém, é
um smell (*Primitive Obsession*): uma chave digitada errada só explode em
tempo de execução e nenhum contrato documenta os campos. Foi decisão
consciente desta aula, para focar no padrão — a Aula 12 troca o dict por uma
`@dataclass Evento` tipada.

## Aula 12 — Refactoring e Code Smells

**Rede de segurança.** O `pytest` funcionou como semáforo em dois momentos
opostos. Quando troquei o evento-`dict` pela `@dataclass Evento`, esqueci de
atualizar um assert do `test_servico_emprestimo.py` que ainda lia
`evento["tipo"]` — a suíte quebrou na hora, apontando arquivo e linha, e o
`TypeError` me disse exatamente o que faltava migrar. Já nos renames e na
extração de `_imprimir_atraso`, a suíte continuou verde a cada passo — e esse
verde é o que **autoriza** dizer que houve refactoring: pela definição de
Fowler (Valente, Cap. 9), refatorar é mudar a estrutura interna **preservando
o comportamento observável**. Sem os testes, eu teria apenas a impressão de
que nada mudou; com eles, tenho evidência a cada commit. Refatorar sem testes
não é refactoring — é mudança arriscada com boas intenções.

**Falso positivo.** `Notebook`, `Projetor` e `Cabo` ficaram vazias depois do
Strategy e, à primeira vista, configuram *Data Class*. Não devem ser
refatoradas: elas são rótulos de tipo por design — o sistema distingue os
tipos, e a fábrica usa essa distinção para injetar a estratégia de multa
certa. Aplicar *Inline Class* dissolveria as subclasses e obrigaria o código
a voltar a perguntar "que tipo é você?" com `if/elif`; devolver o cálculo a
elas desfaria o Strategy e o OCP da Aula 11. Smell é sintoma, não veredito:
reconhecer quando **não** agir também é decisão de engenharia.
